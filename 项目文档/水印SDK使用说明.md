...
```bash
export AGENTMARK_TOOL_MODE=proxy            # proxy: 网关构造 tool_calls; two_pass: 二次调用模型; none: 不生成 tool_calls
export AGENTMARK_TWO_PASS=1                 # 兼容旧开关（等价于 tool_mode=two_pass）
export AGENTMARK_PAYLOAD_BITS=1101          # 固定水印 payload（可选）
export AGENTMARK_SESSION_DEFAULT=demo       # 无 session 时使用的默认会话 key
export AGENTMARK_PROB_TEMPERATURE=2.0       # 概率温度(>1 更平坦)，提高嵌入命中率；默认 1.5
